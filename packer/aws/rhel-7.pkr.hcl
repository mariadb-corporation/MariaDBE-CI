...
variable "ami_id" {
  type    = string
  default = "ami-0302c1ecc74930ba5"
}

source "amazon-ebs" "rhel-7" {
  profile = "jenkins-ci"
  ami_name = "rhel-7-arm64"
...
build {
  sources = ["source.amazon-ebs.rhel-7"]
  provisioner "shell" {
    inline = [
        "sleep 120",
        ...
        "rm -f /tmp/cmake-3.19.8-Linux-aarch64.sh", "sudo yum -y install https://es-repo.mariadb.net/es-ci-files/jdk-11.0.11_linux-aarch64_bin.rpm",
    ]
  }
}
